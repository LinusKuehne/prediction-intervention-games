"""Stabilized Regression -- per-subset score sweep (dataset: sr_TA40_ET.csv, Y = ET).

This script computes the per-subset invariance / predictiveness / skill scores
and writes them to a CSV.  It is meant to be run unattended (overnight) and
interrupted with Ctrl-C when time runs out.  Subsets are processed in batches;
after each batch its rows are flushed to disk, so the CSV holds every completed
batch.  Ctrl-C stops the run cleanly -- only the in-progress batch (at most
CHUNK_SIZE subsets) is lost.  Selecting the stabilized-regression ensemble and
making plots are done afterwards, separately, from that CSV.

Data
----
A single CSV; the ``split`` column assigns rows to train / val / test.  The
environment label is ``cluster`` for the training set and ``env`` for val/test.
Covariates are every column except ET / env / time / cluster / split; the
``PFT_*`` one-hot columns are bundled into one logical covariate, giving
12 continuous + 1 PFT bundle = 13 logical covariates (2^13 = 8192 subsets).

To keep the run tractable the training set is subsampled to N_PER_CLUSTER rows
per cluster (seeded, so the run is repeatable).  val / test are kept in full.

Sweep
-----
All 8192 subsets are listed, shuffled (seeded), and the first MAX_SUBSETS are
scored in parallel (joblib, one subset per task, 250-tree out-of-bag random
forests).  For each subset the worker returns:

* invariance score -- variance across training clusters of the regret
  regret_e(S) = g_all(S, e) - g_e(S, e); the per-cluster regrets are also
  stored (g_all / g_e are OOB RMSEs of forests fit on all clusters / cluster e);
* predictiveness -- pooled OOB RMSE on train; median per-environment RMSE on
  val and on test;
* skill score on the test set -- the external ``skill_score`` (from
  ``stab_reg_eval``) applied to the subset's test-set predictions.

Each finished batch of subsets is appended to SCORES_CSV.
"""
...
MAX_SUBSETS = 4000  # stop after this many subsets (the run is also Ctrl-C-safe)
...
# ---------------------------------------------------------------------------
# list all subsets, shuffle, and keep the first MAX_SUBSETS
# ---------------------------------------------------------------------------
...
subsets = [all_subsets[i] for i in order[:MAX_SUBSETS]]

# variance of Y within each training environment
env_var_y = {e: float(np.var(y_train[E_train == e])) for e in env_values}
print("\nVariance of Y per training environment (cluster):")
for e in env_values:
    print(f"  cluster {e}: {env_var_y[e]:.4f}")

# ---------------------------------------------------------------------------
# sweep the subsets in parallel, streaming each finished subset to the CSV
# ---------------------------------------------------------------------------
...
print(
    f"\nScoring {len(subsets)} subsets (shuffled, of {len(all_subsets)}) "
    f"on {N_JOBS} workers, in batches of {CHUNK_SIZE}; writing to {SCORES_CSV}\n"
...
with open(SCORES_CSV, "w", newline="") as f:
    ...
    try:
        ...
        for start in range(0, len(subsets), CHUNK_SIZE):
            batch = subsets[start : start + CHUNK_SIZE]
            batch_results = cast(
                "list[dict]",
            ...
            n_done += len(batch)
            print(f"  {n_done}/{len(subsets)} subsets written")
    except KeyboardInterrupt:
        print(
            f"\nInterrupted -- {n_done} subsets written; the in-progress "
            f"batch of up to {CHUNK_SIZE} was dropped."
        )
    else:
        print(f"\nDone -- all {n_done} subsets written.")

print(f"Results in {SCORES_CSV} (use it for plots and ensemble selection).")
