...
def _ensure_daily(df, mask, min_contribution_hour_to_day=12):
    ...
    df_daily = aggregate_daily(
        df, mask=mask, min_contribution=min_contribution_hour_to_day
    )
...
def _agg_with_threshold(
    df, groupby_cols, min_contribution, method="mean", early_masking=False
):
    """
    ...
    if min_contribution < 1:
        if early_masking:
            raise NotImplementedError(
                "Early masking with weighted threshold is not implemented yet."
            )
        ...
        frac_true = result["_valid_abs_sum_true"] / result["_abs_sum_true"].replace(
            0, np.nan
        )
        frac_pred = result["_valid_abs_sum_pred"] / result["_abs_sum_pred"].replace(
            0, np.nan
        )
...
def aggregate_weekly(
    df, mask=None, min_contribution=4, min_contribution_hour_to_day=12
):
    """
...
def aggregate_monthly(
    df, mask=None, min_contribution=15, min_contribution_hour_to_day=12
):
    """
...
def aggregate_yearly(
    df, mask=None, min_contribution=183, min_contribution_hour_to_day=12
):
    """
...
def compute_msc(
    ...
    if return_outlier_mask:
        raise DeprecationWarning(
            "Outlier detection is not yet implemented in this function."
        )

    agg_func = method if method in ["mean", "median"] else "mean"

    # Compute MSC per (env, doy)
    msc = _agg_with_threshold(
        df, ["env", "doy"], min_contribution, method=agg_func, early_masking=True
    )
...
def aggregate_seasonal(
    df, mask=None, min_contribution=2, min_contribution_hour_to_day=12, method="mean"
):
    """
    ...
    """
    df, mask = _ensure_daily(df, mask, min_contribution_hour_to_day)
    return compute_msc(df, mask=mask, min_contribution=min_contribution, method=method)


def aggregate_anomaly(
    df, mask=None, min_contribution=2, min_contribution_hour_to_day=12, method="mean"
):
    """
    ...
    # Compute MSC
    msc = compute_msc(df, mask=None, min_contribution=min_contribution, method=method)
    msc = msc.rename(columns={"y_true": "msc_true", "y_pred": "msc_pred"})

    # Merge and compute anomalies
    result = df.merge(
        msc[["env", "doy", "msc_true", "msc_pred"]], on=["env", "doy"], how="left"
    )
...
def aggregate_iav(df, mask=None, min_contribution=183, min_contribution_hour_to_day=12):
    """
